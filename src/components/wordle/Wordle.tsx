...
const GuessLetter: React.FC<GuessLetterProps> = ({ guessed, letter, guess, word }) => {
  const [isInWord, setIsInWord] = useState(false);
  const [isInPlace, setIsInPlace] = useState(false);
...
const GuessWord: React.FC<GuessWordProps> = ({ guess, guesses, word, active, noClear }) => {
  ...
  return (
    <motion.div animate={controls} style={styles.guessWordRow}>
      {savedGuess.split('').map((letter, index) => (
        <GuessLetter guessed={!active} key={index} letter={letter} guess={savedGuess} word={word} />
      ))}
      {[...Array(word.length - savedGuess.length)].map((e, i) => (
        <GuessLetter guessed={!active} key={i} letter={' '} guess={savedGuess} word={word} />
      ))}
    </motion.div>
  );
};
